...
function showResults(){
 ...
 "Coherencia humana: "+(100-(Math.max(...details.map(d=>d.percent),0)-Math.min(...details.map(d=>d.percent),100)))+"%";
...
function renderTips(global,details){
 ...
 details.forEach(d=>{
  ...
  li.innerHTML=`En <strong>${d.area}</strong>, tu nivel sugiere que podrías revisar cómo estás habitando ese espacio y qué tan presente estás en tus acciones cotidianas.`;
  tips.appendChild(li);
 });
}
...
function restart(){showSection("start");document.body.className="";}
